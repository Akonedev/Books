...
import com.enko.books.Borrow.Borrow;
import com.enko.books.Borrow.BorrowRepository;
import com.enko.books.User.UserInfo;
import com.enko.books.User.UserRepository;
...
public class BookController {
    ...
    public ResponseEntity<List<Book>> list(@RequestParam(required = false) BookStatus status, Principal principal) {
    ...
    public ResponseEntity<Object> create(@RequestBody @Valid @NotNull Book book, Principal principal) {
        Integer userConnectedId = this.getUserConnectedId(principal);
        ...
        if (user.isPresent()) {
            ...
            return new ResponseEntity<Object>("You must provide a valid user", HttpStatus.BAD_REQUEST);
        }
